...
def projpc(data,frame_points,intid2inboxpoints,waymoid2intid,idx,output_path,resomult=0.5,multiframe_num=2,proj_territory_size=4,mismatchnframe=4):
    ...
    if (not idx+mismatchnframe<multiframe_num) and (not  idx+mismatchnframe>len(frame_points)-1-multiframe_num):
        merged_lidar_points = np.concatenate(frame_points[ idx+mismatchnframe-multiframe_num: idx+mismatchnframe+multiframe_num+1],axis=0)
    elif  idx+mismatchnframe<multiframe_num:
        merged_lidar_points = np.concatenate(frame_points[0:2*multiframe_num+1],axis=0)
    elif  idx+mismatchnframe>len(frame_points)-1-multiframe_num:
    ...
    for label in frame.laser_labels:
        ...
        if (not idx+mismatchnframe<multiframe_num) and (not  idx+mismatchnframe>len(frame_points)-1-multiframe_num):
            lrange= idx+mismatchnframe-multiframe_num
            rrange= idx+mismatchnframe+multiframe_num+1
        elif  idx+mismatchnframe<multiframe_num:
            lrange=0
...
# import torch
def save_images_from_waymo(segment_path, output_path, nframes=2, interval=2,mismatchnframe=4):
...
parser.add_argument('--output_dir', type=str,default='waymo_pseudoimg_multiframe_+4frame')
...
parser.add_argument('--mismatchnframe', type=int, default=4)
...
if __name__ == "__main__":
    ...
    for idx,tfrecord_path in enumerate(scenes):
        if idx%args.nprocess==fileidx:#可以手动多线程 # manulally multi-process
        
            file_path = os.path.join(folder_path, tfrecord_path)

            save_images_from_waymo(file_path, output_dir, args.nframes, args.interval, mismatchnframe=args.mismatchnframe)
